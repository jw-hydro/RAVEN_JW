...
double CEnthalpyModel::GetEnergyLossesFromLake(const int p, double &Q_sens, double &Q_cond, double &Q_lat, double &Q_rad, double &Q_rain) const
{
  ...
  double Vh_new = pRes->GetHypolimnionStorage();
  double Vh_old = pRes->GetOldHypolimnionStorage();
  ...
  double Ah_new = pRes->GetMixingArea();
  double Ah_old = pRes->GetOldMixingArea();
  double Ah_avg = 0.5 * (Ah_new + Ah_old);
//  double Q_vert = Vh_new-Vh_old;
  
//  if (Q_vert>0) {
//	  double Q_d = Q_Vert; double Q_up = 0;
//  }else{
//	  double Q_d = 0; double Q_up = -Q_Vert;
  ...
  double Te_new =ConvertVolumetricEnthalpyToTemperature(_aMres_epi[p]      / (V_new-Vh_new));
  double Te_old =ConvertVolumetricEnthalpyToTemperature(_aMres_epi_last[p] / (V_old-Vh_old));
  double Th_new =ConvertVolumetricEnthalpyToTemperature(_aMres_hyp[p]      / Vh_new);
  double Th_old =ConvertVolumetricEnthalpyToTemperature(_aMres_hyp_last[p] / Vh_old);
  
  Q_sens=hstar* A_avg * (temp_air -0.5*(Te_new+Te_old));
  Q_cond=hstar * Ah_avg * (0.5*(Th_new+Th_old)- 0.5 * (Te_new + Te_old));
  Q_rad =(SW+LW)*A_avg;
  Q_lat =-(AET * DENSITY_WATER * LH_VAPOR)*A_avg;
  Q_rain=_aMresRain[p];
  Q_OUT =-_aMout_res[p];

  return -(Q_sens + Q_OUT + Q_cond  + Q_rad +  Q_lat + Q_rain) * tstep; //[MJ]
}
...
// N-R solution of Crank-nicolson problem as set of two non-linear algebraic equations
  ...
  // -----------------------------------------------------------------------------------------
  //dE   /dt=Qh_in-Qh_out-Qh_d+Qh_up+As*(Rnet+P*hrain-ET*rho*LH)+k*(Tair-T(E))*As+k*(Te-Th)*Ah)    //[MJ/d]
  //dEsed/dt=                                                         QH_d-Qh_up+(Tsed-T(E))*Ah     //[MJ/d]
...
//B[0] = 0.5*(aMout_new[nSegments-1]+_aMout[p][nSegments-1])*tstep; // inflow [MJ]
  B[0]=[1-0.5*(aMout_new[p][nSegments-1]/(V_old-Vh_old))*tstep]*_aMres_epi[p];
  ...
  B[0]-=0.5*(hstar*A_avg+Ah_avg*ksed)*tstep*Te_old;
  B[0]+=0.5*Ah_avg*ksed*Th_old;
  
  
  B[1] =0.5*ksed*Ah_avg *Te_old;
  B[1]-=0.5*tstep*ksed *Ah_avg*Th_old;
  ...
  double Qh_d=0;
  double Qh_up=0;
  double Te_guess,Th_guess;
  double dHe,dHh;
  double He_guess  =_aMres_epi_last[p];
  double Hh_guess =_aMres_hyp_last[p];
  
  while (change > tolerance) 
  {
    Te_guess = ConvertVolumetricEnthalpyToTemperature[He_guess/(V_new-Vh_new)];
    Th_guess= ConvertVolumetricEnthalpyToTemperature(Hh_guess/Vh_new);

    A[0][0] = 1.0 + 0.5 * tstep  * Q_new/V_new-Vh_new;
    A[1][1] = 1.0;
    A[1][0] = 0.0;
    A[0][1] = 0.0;
    if (Te_guess!=0.0){
      A[0][0]+= 0.5*tstep*hstar*A_avg*Te_guess/He_guess;
      A[0][0]+= 0.5*tstep*ksed *Ah_avg*Te_guess/He_guess;  //[MJ/m2/d/K]*[m2]/[MJ/m3/K] = [m3/d]  
      A[1][0]-= 0.5*tstep*ksed *Ah_avg*Te_guess/He_guess;
    }
    if (Th_guess!=0.0){
      A[0][1]+= 0.5*tstep*ksed *Ah_avg*Th_guess/Hh_guess;
      A[1][1]+= 1+0.5*tstep*ksed *Ah_avg*Th_guess/Hh_guess;
    }

    //J_ij=A_ij+E*dA_ij/dE = Jacobian
    J[0][0]=A[0][0]+0.5*tstep*(hstar*A_avg*ksed*Ah_avg) * (TemperatureEnthalpyDerivative(He_guess /(V_new-Vh_new))/ (V_new-Vh_new));
    J[0][1]=A[0][1]+0.5*tstep*(            ksed*Ah_avg) * (TemperatureEnthalpyDerivative(Hh_guess/Vh_new )/ Vh_new );
    J[1][0]=A[1][0]-0.5*tstep*(            ksed*Ah_avg) * (TemperatureEnthalpyDerivative(He_guess /(V_new-Vh_new))/ (V_new-Vh_new));
    J[1][1]=A[1][1]+0.5*tstep*(            ksed*Ah_avg) * (TemperatureEnthalpyDerivative(Hh_guess/Vh_new )/ Vh_new );

    R[0]   =-A[0][0]*He_guess-A[0][1]*Hh_guess+B[0];
    R[1]   =-A[1][0]*Te_guess-A[1][1]*Th_guess+B[1];

    //invert 2x2 matrix analytically
    double den= (J[0][1]*J[1][0]-J[0][0]*J[1][1]);
    dHe  = (J[0][1]*R[1]-J[1][1]*R[0])/den;
    dHh = (J[1][0]*R[0]-J[0][0]*R[1])/den;

    change =sqrt(dHe*dHe+dHh*dHh)/ V_new / HCP_WATER; //convert to approx temp difference (for >0C water)
